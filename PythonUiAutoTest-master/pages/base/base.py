...
class Base(object):
    ...
    def __init__(self, driver: webdriver, timeout=30):
        ...
        self.base_url = 'http://audit.k8s.superlucy.net/#/user/sign-in'
...
if __name__ == '__main__':
    ...
    d = webdriver.Chrome(options=chrome_options,executable_path=mydrvier)
    d.implicitly_wait(2) #隐式等待
    # d = webdriver.Chrome()
    b = Base(d)
    b.open('http://audit.k8s.superlucy.net/#/user/sign-in')
    print('5')
    sleep(5)
    d.maximize_window()  # 最大化窗口
    b.find_element('xpath', '//input[@id="loginName"]').send_keys('sdl1')
    b.find_element('xpath', '//input[@id="password"]').send_keys('sdl@123')
    b.find_element('xpath', '//button[@class="ant-btn ant-btn-primary"]').click()
